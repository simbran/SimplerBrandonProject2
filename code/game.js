...
Player.prototype.act = function(step, level, keys) {
  ...
  // Losing animation
  if (level.status == "lostlava" || level.status == "lostspike" || level.status == "lostenemy") {
    this.pos.y += step;
    this.size.y -= step;
  }
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    this.status = "lostlava";
    this.finishDelay = 1;
	document.getElementById("p").innerHTML = "You burnt to a crisp in the lava.";
  } 
    else if (type == "spike" && this.status == null) {
    this.status = "lostspike";
    this.finishDelay = 1;
	document.getElementById("p").innerHTML = "You were speared by the spike. Ouch.";
  }
    else if (type == "enemy" && this.status == null) {
    this.status = "lostenemy";
    this.finishDelay = 1;
	document.getElementById("p").innerHTML = "You were caught by the enemies.";
  }  
    else if (type == "coin") {
...
function runGame(plans, Display) {
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lostlava" || status == "lostspike" || status == "lostenemy")
        startLevel(n);
      else if (n < plans.length - 1)
        startLevel(n + 1);
      else {
        console.log("You win!");
	    document.getElementById("p").innerHTML = "Congradulations!! You beat the second project dragon!! =D" }
    });
  }
  startLevel(0);
}
